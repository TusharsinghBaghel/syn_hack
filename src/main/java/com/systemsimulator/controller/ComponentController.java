...
public class ComponentController {
    ...
    /** Get component by ID */
    @GetMapping("/{id}")
    public ResponseEntity<Object> getComponentById(@PathVariable String id) {
    ...
    @PostMapping
    public ResponseEntity<Object> createComponent(@RequestBody ComponentRequest request) {
        try {
            ...
            logger.info("Successfully created component: id={}, type={}, name={}",
                    id, request.getType(), request.getName());
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Object> updateComponent(@PathVariable String id, @RequestBody Component component) {
        ...
        logger.info("Updated component: id={}, name={}", id, component.getName());
        return ResponseEntity.ok(updated);
    }

    /** Delete a component */
    @DeleteMapping("/{id}")
    public ResponseEntity<Object> deleteComponent(@PathVariable String id) {
        boolean deleted = componentService.deleteComponent(id);
        if (!deleted) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ErrorResponse("Component not found: " + id));
        }
        logger.info("Deleted component: id={}", id);
    ...
    /** Get all component types (enum values) */
    @GetMapping("/types")
    ...
    public ResponseEntity<SubtypesResponse> getSubtypesForType(@PathVariable ComponentType type) {
        List<String> subtypes;

        switch (type) {
            case DATABASE -> subtypes = java.util.Arrays.stream(DatabaseComponent.DatabaseType.values())
                    .map(Enum::name).toList();
            case CACHE -> subtypes = java.util.Arrays.stream(CacheComponent.CacheType.values())
                    .map(Enum::name).toList();
            case API_SERVICE -> subtypes = java.util.Arrays.stream(APIServiceComponent.APIType.values())
                    .map(Enum::name).toList();
            case QUEUE -> subtypes = java.util.Arrays.stream(QueueComponent.QueueType.values())
                    .map(Enum::name).toList();
            case STORAGE -> subtypes = java.util.Arrays.stream(StorageComponent.StorageType.values())
                    .map(Enum::name).toList();
            case LOAD_BALANCER -> subtypes = java.util.Arrays.stream(LoadBalancerComponent.LoadBalancerType.values())
                    .map(Enum::name).toList();
            default -> subtypes = List.of("default");
        }

        return ResponseEntity.ok(new SubtypesResponse(type, subtypes));
    }

    /** Get heuristic profile preview for frontend */
    @GetMapping("/heuristics/{type}/{subtype}")
    public ResponseEntity<HeuristicProfile> getHeuristicsForTypeAndSubtype(
