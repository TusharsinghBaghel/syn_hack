...
public class LinkController {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<Object> getLinkById(@PathVariable String id) {
    ...
    public ResponseEntity<ValidationResponse> validateLink(@RequestBody LinkValidationRequest request) {
        var result = linkService.validateLinkWithReason(
                request.getSourceId(),
                request.getTargetId(),
                request.getLinkType()
        );

        return ResponseEntity.ok(new ValidationResponse(result.isValid(), result.getReason()));
    }
    ...
    public ResponseEntity<?> getSuggestions(@RequestBody SuggestionRequest request) {
        ...
        if (sourceOpt.isEmpty() || targetOpt.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ErrorResponse("One or both components not found."));
        }
    ...
    public ResponseEntity<List<Link>> getLinksForComponent(@PathVariable String componentId) {
        return ResponseEntity.ok(linkService.getLinksForComponent(componentId));
    }
    ...
    public ResponseEntity<?> getConnectionStats(@PathVariable String componentId) {
        try {
            return ResponseEntity.ok(linkService.getConnectionStats(componentId));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
    ...
    public ResponseEntity<CountResponse> getLinkCount() {
        return ResponseEntity.ok(new CountResponse(linkService.getAllLinks().size()));
    }
    ...
    public ResponseEntity<DeleteResponse> deleteLinksForComponent(@PathVariable String componentId) {
        int deletedCount = linkService.getLinksForComponent(componentId).size();
    ...
    public ResponseEntity<HeuristicProfile> getDefaultHeuristicsForLinkType(@PathVariable LinkType linkType) {
        return ResponseEntity.ok(heuristicService.getDefaultHeuristicsForLinkType(linkType));
    }
    ...
    public ResponseEntity<?> calculateLinkScore(
            @PathVariable String id,
            @RequestBody Map<Parameter, Double> weights) {
        Optional<Link> linkOpt = linkService.getLinkById(id);
        if (linkOpt.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ErrorResponse("Link not found: " + id));
